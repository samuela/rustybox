...
unsafe extern "C" fn string_checker_helper(
  ...
  /* as sub-string */
  if !strcasestr(p2, p1).is_null() || !strcasestr(p1, p2).is_null() {
...
unsafe extern "C" fn obscure_msg(
  ...
  if new_p.is_null() || {
    ...
    (length) < 6i32 as libc::c_uint
  } {
    return b"too short\x00" as *const u8 as *const libc::c_char;
  }
  /* no username as-is, as sub-string, reversed, capitalized, doubled */
  if string_checker(new_p, (*pw).pw_name) != 0 {
    return b"similar to username\x00" as *const u8 as *const libc::c_char;
  }
  /* no gecos as-is, as sub-string, reversed, capitalized, doubled */
  if *(*pw).pw_gecos.offset(0) as libc::c_int != 0 && string_checker(new_p, (*pw).pw_gecos) != 0 {
  ...
  while i < length {
    ...
    {
      /* a-z */
      mixed |= 1i32 as libc::c_uint
    } else if (*new_p.offset(i as isize) as libc::c_int - 'A' as i32) as libc::c_uchar
      as libc::c_int
      <= 'Z' as i32 - 'A' as i32
    {
      /* A-Z */
      mixed |= 2i32 as libc::c_uint
    } else if (*new_p.offset(i as isize) as libc::c_int - '0' as i32) as libc::c_uchar
      as libc::c_int
      <= 9i32
    {
      mixed |= 4i32 as libc::c_uint
    } else {
      mixed |= 8i32 as libc::c_uint
    } /* special characters */
    /* 0-9 */
    /* Count i'th char */
    ...
    if c.wrapping_mul(2i32 as libc::c_uint) >= length {
      return b"too many similar characters\x00" as *const u8 as *const libc::c_char;
    }
    i = i.wrapping_add(1)
  }
  size = (6i32 + 2i32 * 4i32) as libc::c_uint;
  i = 1i32 as libc::c_uint;
  while i <= 8i32 as libc::c_uint {
    if mixed & i != 0 {
      size = size.wrapping_sub(2i32 as libc::c_uint)
    }
    i <<= 1i32
  }
  if length < size {
    return b"too weak\x00" as *const u8 as *const libc::c_char;
  }
  if !old_p.is_null() && *old_p.offset(0) as libc::c_int != 0 {
    /* check vs. old password */
    if string_checker(new_p, old_p) != 0 {
