...
*/
unsafe extern "C" fn comm_match(
  mut p: *mut procps_status_t,
   ...
   * no way to know that) */
  if (*p).comm[14] as libc::c_int == '\u{0}' as i32 {
    return 1i32;
  } /* comm is not truncated - matches */
  ...
  loop {
    p = procps_scan(
      ...
      PSSCAN_PID as libc::c_int
        ...
        | PSSCAN_ARGVN as libc::c_int
        | PSSCAN_EXE as libc::c_int,
    );
    if p.is_null() {
      break;
    }
