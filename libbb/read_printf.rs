...
pub unsafe fn xmalloc_read_with_initial_buf(
  ...
  let mut size: size_t = 0; /* max to read */
  let mut rd_size: size_t = 0;
  let mut to_read: size_t = 0;
  let mut st: stat = std::mem::zeroed();
  to_read = if !maxsz_p.is_null() {
    *maxsz_p
  } else {
    (2147483647i32 - 4095i32) as libc::c_ulong
  };
  /* Estimate file size */
  st.st_size = 0 as off_t; /* in case fstat fails, assume 0 */
  fstat(fd, &mut st);
  ...
  loop {
    ...
    to_read = (to_read as libc::c_ulong).wrapping_sub(rd_size) as size_t as size_t;
    /* grow by 1/8, but in [1k..64k] bounds */
    size = (total.wrapping_div(8i32 as libc::c_ulong) | 0x3ffi32 as libc::c_ulong)
      .wrapping_add(1i32 as libc::c_ulong);
    if size > (64i32 * 1024i32) as libc::c_ulong {
  ...
  *buf.offset(total as isize) = '\u{0}' as i32 as libc::c_char;
  if !maxsz_p.is_null() {
    *maxsz_p = total
  }
