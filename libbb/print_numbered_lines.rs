...
pub unsafe extern "C" fn print_numbered_lines(
  ...
  loop {
    ...
    if (*ns).all as libc::c_int != 0
      || (*ns).nonempty as libc::c_int != 0 && *line.offset(0) as libc::c_int != 0
    {
      printf(
      ...
      N = N.wrapping_add((*ns).inc)
    } else if !(*ns).empty_str.is_null() {
      fputs_unlocked((*ns).empty_str, stdout);
    }
