...
pub unsafe extern "C" fn fputc_printable(mut ch: libc::c_int, mut file: *mut FILE) {
  ...
  ch = ch as libc::c_uchar as libc::c_int;
  if ch == 0x9bi32 {
    /* VT100's CSI, aka Meta-ESC, is not printable on vt-100 */
    ch = '{' as i32;
    current_block = 13227821086609925615;
  } else if ch < ' ' as i32 {
    ch += '@' as i32;
    current_block = 13227821086609925615;
  } else if ch == 0x7fi32 {
  ...
  match current_block {
    13227821086609925615 => {
      putc_unlocked('^' as i32, file);
    }
    _ => {}
  }
...
pub unsafe extern "C" fn visible(
  ...
  if !(ch == '\t' as i32 as libc::c_uint && flags & VISIBLE_SHOW_TABS as libc::c_int == 0) {
    if ch == '\n' as i32 as libc::c_uint {
      ...
      if ch >= 128i32 as libc::c_uint {
        ch = ch.wrapping_sub(128i32 as libc::c_uint);
        let fresh1 = buf;
        buf = buf.offset(1);
        *fresh1 = 'M' as i32 as libc::c_char;
        let fresh2 = buf;
        buf = buf.offset(1);
        *fresh2 = '-' as i32 as libc::c_char
      }
      if ch < 32i32 as libc::c_uint || ch == 127i32 as libc::c_uint {
        let fresh3 = buf;
        buf = buf.offset(1);
        *fresh3 = '^' as i32 as libc::c_char;
        ch ^= 0x40i32 as libc::c_uint
      }
    }
  }
  let fresh4 = buf;
  buf = buf.offset(1);
  *fresh4 = ch as libc::c_char;
