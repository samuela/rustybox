...
pub unsafe extern "C" fn setsockopt_bindtodevice(
  ...
  strncpy_IFNAMSIZ(ifr.ifr_ifrn.ifrn_name.as_mut_ptr(), iface);
...
unsafe extern "C" fn get_lsa(
  ...
  let mut lsa: len_and_sockaddr = len_and_sockaddr {
    len: 0,
    u: C2RustUnnamed_2 {
      sa: sockaddr {
        sa_family: 0,
...
pub unsafe extern "C" fn bb_lookup_port(
  ...
  if !port.is_null() {
    ...
    old_errno = *bb_errno;
    port_nr = bb_strtou(port, 0 as *mut *mut libc::c_char, 10i32);
...
unsafe extern "C" fn str2sockaddr(
  ...
  if !cp.is_null() {
    ...
    match current_block_34 {
      8693738493027456495 => {
        cp = cp.offset(1);
        port = bb_strtou(cp, 0 as *mut *mut libc::c_char, 10i32) as libc::c_int;
     ...
     * check whether this is a numeric IPv6 */
    {
      if af as libc::c_int != 2i32 {
        ...
        if inet_pton(10i32, host, &mut in6 as *mut in6_addr as *mut libc::c_void) > 0i32 {
          ...
          (*r).len = ::std::mem::size_of::<sockaddr_in6>() as libc::c_ulong as socklen_t;
          (*r).u.sa.sa_family = 10i32 as sa_family_t;
      ...
      match current_block {
        ...
        _ => {
          ...
          if rc != 0 || result.is_null() {
            ...
            r = xmalloc(
              (LSA_LEN_SIZE as libc::c_int as libc::c_uint).wrapping_add((*used_res).ai_addrlen)
                as size_t,
            ) as *mut len_and_sockaddr;
...
pub unsafe extern "C" fn xsocket_type(
  ...
  lsa = xzalloc((LSA_LEN_SIZE as libc::c_int + len) as size_t) as *mut len_and_sockaddr;
...
pub unsafe extern "C" fn create_and_bind_to_netlink(
  ...
  let mut sa: sockaddr_nl = sockaddr_nl {
    nl_family: 0,
    nl_pad: 0,
    nl_pid: 0,
    nl_groups: 0,
