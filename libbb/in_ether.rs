 ...
 */
pub unsafe fn in_ether(mut bufp: *const libc::c_char, mut sap: *mut sockaddr) -> libc::c_int {
  let mut ptr: *mut libc::c_char = std::ptr::null_mut::<libc::c_char>();
  let mut i: libc::c_int = 0;
  let mut j: libc::c_int = 0;
  let mut val: libc::c_uchar = 0;
  let mut c: libc::c_uchar = 0;
  (*sap).sa_family = 1i32 as sa_family_t;
  ...
  loop {
    ...
    loop {
      ...
      } else if ((c as libc::c_int | 0x20i32) - 'a' as i32) as libc::c_uchar as libc::c_int <= 5i32
      {
        c = (((c as libc::c_int | 0x20i32) - 'a' as i32) as libc::c_uchar as libc::c_int + 10i32)
          as libc::c_uchar
      } else {
        if j != 0 && (c as libc::c_int == ':' as i32 || c as libc::c_int == '\u{0}' as i32) {
          break;
        }
        return -1i32;
      }
      bufp = bufp.offset(1);
      val = ((val as libc::c_int) << 4i32) as libc::c_uchar;
      val = (val as libc::c_int + c as libc::c_int) as libc::c_uchar;
      j ^= 1i32;
      if !(j != 0) {
        break;
      }
    ...
    *fresh0 = val as libc::c_char;
    ...
    if *bufp as libc::c_int == ':' as i32 {
      bufp = bufp.offset(1)
    }
  }
  /* Error if we aren't at end of string */
  return *bufp as libc::c_int;
}
