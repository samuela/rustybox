 ...
 */
#[no_mangle]
pub static mut global_crc32_table: *mut u32 = 0 as *mut u32;
#[no_mangle]
pub unsafe extern "C" fn crc32_filltable(
  ...
  while i < 256i32 as libc::c_uint {
    ...
    while j != 0 {
      if endian != 0 {
        c = if c & 0x80000000u32 != 0 {
          (c << 1i32) ^ polynomial
        } else {
          (c) << 1i32
        }
      } else {
        c = if c & 1i32 as libc::c_uint != 0 {
          (c >> 1i32) ^ polynomial
        } else {
          (c) >> 1i32
        }
    ...
    let fresh0 = crc_table;
    crc_table = crc_table.offset(1);
    *fresh0 = c;
    i = i.wrapping_add(1)
  }
