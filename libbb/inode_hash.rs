...
static mut ino_dev_hashtable: *mut *mut ino_dev_hashtable_bucket_t =
  0 as *const *mut ino_dev_hashtable_bucket_t as *mut *mut ino_dev_hashtable_bucket_t;
/*
...
pub unsafe extern "C" fn add_to_ino_dev_hashtable(
  ...
  bucket = xmalloc(
    (::std::mem::size_of::<ino_dev_hashtable_bucket_t>() as libc::c_ulong)
      .wrapping_add(strlen(name)),
  ) as *mut ino_dev_hashtable_bucket_t;
  (*bucket).ino = (*statbuf).st_ino;
  (*bucket).dev = (*statbuf).st_dev;
  (*bucket).isdir = ((*statbuf).st_mode & 0o170000i32 as libc::c_uint == 0o40000i32 as libc::c_uint)
  ...
  i = ((*statbuf).st_ino as libc::c_uint).wrapping_rem(311u32) as libc::c_int;
  (*bucket).next = *ino_dev_hashtable.offset(i as isize);
  let ref mut fresh0 = *ino_dev_hashtable.offset(i as isize);
  *fresh0 = bucket;
}
