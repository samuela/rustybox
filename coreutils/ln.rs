  ...
  */
  ...
  if opts & 1i32 << 6i32 != 0 && argc > 2i32 {
    crate::libbb::verror_msg::bb_simple_error_msg_and_die(
      b"-T accepts 2 args max\x00" as *const u8 as *const libc::c_char,
    );
  }
  if (*argv.offset(1)).is_null() {
    ...
    *argv = last;
     ...
     */
    last = crate::libbb::get_last_path_component::bb_get_last_path_component_strip(
      crate::libbb::xfuncs_printf::xstrdup(last),
    )
  }
  ...
  loop {
    ...
    src = last;
    if crate::libbb::isdirectory::is_directory(
      src,
    ...
    ) != 0
    {
      ...
      src_name = crate::libbb::xfuncs_printf::xstrdup(*argv);
      src = crate::libbb::concat_path_file::concat_path_file(
        src,
        crate::libbb::get_last_path_component::bb_get_last_path_component_strip(src_name),
      );
    ...
    match current_block_45 {
      5689316957504528238 => {
        if opts & 1i32 << 3i32 != 0 {
          ...
          backup = crate::libbb::xfuncs_printf::xasprintf(
            b"%s%s\x00" as *const u8 as *const libc::c_char,
            src,
            suffix,
          );
          if rename(src, backup) < 0 && *bb_errno != 2i32 {
            crate::libbb::perror_msg::bb_simple_perror_msg(src);
            status = 1i32;
