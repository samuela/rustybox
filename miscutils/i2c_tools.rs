...
unsafe extern "C" fn confirm_or_abort() {
  ...
  if crate::libbb::ask_confirmation::bb_ask_y_confirmation() == 0 {
    crate::libbb::verror_msg::bb_simple_error_msg_and_die(
      b"aborting\x00" as *const u8 as *const libc::c_char,
    );
  };
}
/*
...
unsafe extern "C" fn confirm_action(
  ...
  if pec != 0 {
    crate::libbb::verror_msg::bb_simple_error_msg(
      b"PEC checking enabled\x00" as *const u8 as *const libc::c_char,
    );
  }
  confirm_or_abort();
}
...
pub unsafe fn i2cget_main(mut _argc: libc::c_int, mut argv: *mut *mut libc::c_char) -> libc::c_int {
  ...
  match mode {
    ...
    3 => status = i2c_smbus_read_word_data(fd, data_addr as u8),
    _ => {
      /* I2C_SMBUS_BYTE_DATA */
...
pub unsafe fn i2cset_main(mut argc: libc::c_int, mut argv: *mut *mut libc::c_char) -> libc::c_int {
    ...
    if !(((i / 16i32) as libc::c_uint) < first.wrapping_div(16i32 as libc::c_uint)) {
      ...
      while j < 16i32 {
        ...
        if ((i + j) as libc::c_uint) < first || (i + j) as libc::c_uint > last {
          ...
          match mode {
            2 => {
              res = i2c_smbus_read_byte_data(bus_fd, (i + j) as u8);
      ...
      while j < 16i32 {
        ...
        if ((i + j) as libc::c_uint) < first || (i + j) as libc::c_uint > last {
          ...
          if res < 0 {
            ...
            crate::libbb::xfuncs_printf::bb_putchar(res);
          }
        }
        j += 1
      }
...
unsafe extern "C" fn list_i2c_busses_and_exit() -> ! {
   ...
   */
  dir = crate::libbb::xfuncs_printf::xopendir(i2cdev_path);
  loop {
    de = readdir(dir);
    if de.is_null() {
      break;
...
pub unsafe fn i2cdetect_main(
  ...
  if !(*argv.offset(1)).is_null() {
    first = crate::libbb::xatonum::xstrtou_range(*argv.offset(1), 16i32, first, last);
    if !(*argv.offset(2)).is_null() {
      last = crate::libbb::xatonum::xstrtou_range(*argv.offset(2), 16i32, first, last)
    }
  }
...
//usage:     "\n	-y	Disable interactive mode"
pub unsafe fn i2ctransfer_main(
  mut _argc: libc::c_int,
  mut argv: *mut *mut libc::c_char,
) -> libc::c_int {
  let mut bus_num: libc::c_int = 0;
  ...
  opts =
    crate::libbb::getopt32::getopt32(argv, b"^fya\x00-2\x00" as *const u8 as *const libc::c_char);
  ...
  loop {
    ...
    match *fresh2 as libc::c_int {
      114 => flags = (flags as libc::c_int | 0x1i32) as u16,
      119 => {}
      _ => {
        crate::libbb::appletlib::bb_show_usage();
      }
    ...
    if len != 0 {
      msgs[nmsgs as usize].buf = crate::libbb::xfuncs_printf::xzalloc(len as size_t) as *mut __u8
    }
          ...
          *msgs[nmsgs as usize].buf.offset(fresh3 as isize) = data8;
          ...
          match *end as libc::c_int {
            112 => {
              ...
              data8 = ((data8 as libc::c_int ^ 27i32) + 13i32) as u8;
              data8 = ((data8 as libc::c_int) << 1i32 | data8 as libc::c_int >> 7i32) as u8
            }
  ...
  nmsgs_sent = crate::libbb::xfuncs_printf::ioctl_or_perror_and_die(
    fd,
