...
unsafe extern "C" fn search_name_hashtable(mut key: *const libc::c_char) -> libc::c_int {
  ...
  while !(*ptr_to_globals).name_hashtable[probe_address as usize].is_null() {
    ...
    if (probe_address as libc::c_int) < 0i32 {
      probe_address = probe_address.wrapping_add(16381i32 as libc::c_uint)
    }
  }
  (*ptr_to_globals).name_hashtable[probe_address as usize] = xstrdup(key);
  return probe_address as libc::c_int;
}
...
unsafe extern "C" fn test_version(
  ...
  match operator {
    6 => return 1i32,
    1 => return (version_result == 0i32) as libc::c_int,
    2 => return (version_result < 0i32) as libc::c_int,
    3 => return (version_result <= 0i32) as libc::c_int,
    4 => return (version_result > 0i32) as libc::c_int,
    5 => return (version_result >= 0i32) as libc::c_int,
    _ => {}
  }
  return 0i32;
}
...
unsafe extern "C" fn add_edge_to_node(mut node: *mut common_node_t, mut edge: *mut edge_t) {
  (*node).edge = xrealloc_vector_helper(
...
unsafe extern "C" fn add_split_dependencies(
  ...
  loop {
    ...
    {
      or_edge = xzalloc(::std::mem::size_of::<edge_t>() as libc::c_ulong) as *mut edge_t;
...
unsafe extern "C" fn read_package_field(
  ...
  let mut offset_name_start: libc::c_int = 0i32;
  let mut offset_name_end: libc::c_int = 0i32;
  let mut offset_value_start: libc::c_int = 0i32;
  let mut offset_value_end: libc::c_int = 0i32;
...
unsafe extern "C" fn fill_package_struct(mut control_buffer: *mut libc::c_char) -> libc::c_uint {
  ...
  while field_start < buffer_length {
    ...
    if !field_name.is_null() {
      field_num = index_in_strings(field_names.as_ptr(), field_name) as libc::c_uint;
      match field_num {
        0 => {
          /* Package */
...
unsafe extern "C" fn describe_status(mut status_num: libc::c_int) -> *const libc::c_char {
  ...
  if (*ptr_to_globals).status_hashtable[status_num as usize].is_null()
    || (*(*ptr_to_globals).status_hashtable[status_num as usize]).status() as libc::c_int == 0i32
  {
...
unsafe extern "C" fn index_status_file(mut filename: *const libc::c_char) {
  ...
  loop {
    control_buffer =
      xmalloc_fgetline_str(status_file, b"\n\n\x00" as *const u8 as *const libc::c_char);
...
unsafe extern "C" fn write_status_file(mut deb_file: *mut *mut deb_file_t) {
  let mut old_status_file: *mut FILE =
  ...
  {
    control_buffer = xmalloc_fgetline_str(
      old_status_file,
      b"\n\n\x00" as *const u8 as *const libc::c_char,
    );
    if control_buffer.is_null() {
    ...
    if !(*ptr_to_globals).status_hashtable[status_num as usize].is_null() {
      ...
      if strcmp(status_from_file, status_from_hashtable) != 0i32 {
          ...
          ) == 0i32
        {
          ...
          if (*deb_file.offset(i as isize)).is_null() {
            bb_error_msg_and_die(b"ALERT: cannot find a control file, your status file may be broken, status may be incorrect for %s\x00"
                                                 as *const u8 as
                                                 *const libc::c_char,
                                             package_name);
          }
...
/*, int dep_max_count - ?? */ {
  ...
  while !(*deb_file.offset(i as isize)).is_null() {
    ...
    conflicts = xrealloc_vector_helper(
  ...
  while !(*deb_file.offset(i as isize)).is_null() {
    ...
    let mut status_num: libc::c_int = 0i32;
    status_num =
          ...
          while j < (*package_node_0).num_of_edges() as libc::c_int {
              ...
              || (*package_edge_0).type_0() as libc::c_int == EDGE_OR_DEPENDS as libc::c_int
            {
                ...
                || (*package_edge_0).type_0() as libc::c_int == EDGE_DEPENDS as libc::c_int
              {
                ...
                if result_0 != 0 && number_of_alternatives == 0i32 {
                  if !root_of_alternatives.is_null() {
                    bb_error_msg_and_die(
                ...
                if result_0 == 0i32 && number_of_alternatives != 0 {
...
unsafe extern "C" fn create_list(mut filename: *const libc::c_char) -> *mut *mut libc::c_char {
  ...
  count = 0i32;
  loop {
    line = xmalloc_fgetline(list_stream);
    if line.is_null() {
      break;
    }
    file_list = xrealloc_vector_helper(
...
unsafe extern "C" fn remove_file_array(
  ...
  while !(*remove_names.offset(i as isize)).is_null() {
    ...
    match current_block_9 {
      ...
      {
        if !(lstat(*remove_names.offset(i as isize), &mut path_stat) < 0i32) {
  ...
  return (remove_flag == 0i32) as libc::c_int;
}
unsafe extern "C" fn run_package_script_or_die(
  ...
  result = if access(script_path, 0i32) != 0 {
    0i32
  } else {
    system(script_path)
  };
  ...
  while i < 8191i32 + 1i32 {
    if !(*ptr_to_globals).status_hashtable[i as usize].is_null() {
      ...
      vers_str = (*ptr_to_globals).name_hashtable[(*(*ptr_to_globals).package_hashtable
        [(*(*ptr_to_globals).status_hashtable[i as usize]).package() as usize])
        .version() as usize];
      if !(!pattern.is_null() && fnmatch(pattern, name_str, 0i32) != 0i32) {
...
unsafe extern "C" fn init_archive_deb_ar(
  ...
  (*ar_handle).src_fd = xopen(filename, 0i32);
  return ar_handle;
}
...
unsafe extern "C" fn data_extract_to_buffer(mut archive_handle: *mut archive_handle_t) {
  ...
  (*archive_handle).dpkg__buffer =
    xzalloc(size.wrapping_add(1i32 as libc::c_uint) as size_t) as *mut libc::c_char;
  xread(
...
unsafe extern "C" fn deb_extract_control_file_to_buffer(
  ...
  (*(*ar_handle).dpkg__sub_archive).filter =
...
unsafe extern "C" fn append_control_file_to_llist(
  ...
  if !fp.is_null() {
    loop {
      ...
      llist_add_to(ll, line as *mut libc::c_void);
    }
    fclose(fp);
  };
}
unsafe extern "C" fn filter_rename_config(
  ...
  if find_list_entry((*archive_handle).accept, name_ptr).is_null() {
    return 0i32 as libc::c_char;
  } /* no */
  fd = open(name_ptr, 0i32);
  if fd >= 0i32 {
    ...
    md5_begin(&mut md5);
    loop {
      count = safe_read(fd, buf as *mut libc::c_void, 4096i32 as size_t) as libc::c_int;
      if !(count > 0i32) {
        break;
      }
      md5_hash(&mut md5, buf as *const libc::c_void, count as size_t);
    }
    md5_end(&mut md5, buf as *mut libc::c_void);
    close(fd);
    md5line = xmalloc(
    ...
    sprintf(
      bin2hex(md5line, buf, 16i32),
      b"  %s\x00" as *const u8 as *const libc::c_char,
      name_ptr,
    );
    free(buf as *mut libc::c_void);
    /* Is it changed after install? */
    if find_list_entry((*archive_handle).accept, md5line).is_null() {
      printf(
        b"Warning: Creating %s as %s.dpkg-new\n\x00" as *const u8 as *const libc::c_char,
        name_ptr,
        name_ptr,
      );
      (*(*archive_handle).file_header).name = xasprintf(
...
unsafe extern "C" fn unpack_package(mut deb_file: *mut deb_file_t) {
  ...
  ) == 0i32
  {
    ...
    remove_package(status_package_num, 0i32);
  } else {
    printf(
  ...
  while (i as libc::c_uint)
    < (::std::mem::size_of::<[*const libc::c_char; 10]>() as libc::c_ulong)
      ...
      as libc::c_uint
  {
    let mut c: *mut libc::c_char = xasprintf(
      b"./%s\x00" as *const u8 as *const libc::c_char,
      all_control_files[i as usize],
    );
    llist_add_to(&mut accept_list, c as *mut libc::c_void);
    i += 1
  }
  ...
  while !(*(*archive_handle).dpkg__sub_archive).passed.is_null() {
    let mut filename: *mut libc::c_char =
      llist_pop(&mut (*(*archive_handle).dpkg__sub_archive).passed) as *mut libc::c_char;
...
pub unsafe extern "C" fn dpkg_main(
  ...
  *fresh11 = xzalloc(::std::mem::size_of::<globals>() as libc::c_ulong) as *mut globals;
  ...
  if opt & OPT_force as libc::c_int != 0 {
    ...
    if strcmp(str_f, b"depends\x00" as *const u8 as *const libc::c_char) == 0i32 {
      opt |= OPT_force_ignore_depends as libc::c_int
    } else if strcmp(str_f, b"confnew\x00" as *const u8 as *const libc::c_char) == 0i32 {
      opt |= OPT_force_confnew as libc::c_int
    } else if strcmp(str_f, b"confold\x00" as *const u8 as *const libc::c_char) == 0i32 {
  ...
  while !(*argv).is_null() {
    ...
    deb_file = xrealloc_vector_helper(
    ...
    *fresh12 = xzalloc(::std::mem::size_of::<deb_file_t>() as libc::c_ulong) as *mut deb_file_t;
